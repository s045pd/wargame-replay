...
- **导播模式** — AI 辅助镜头切换，多预览窗格，自动追踪热点事件，聚焦模式（暗色地图 + 减速）
- **时间轴与回放** — 多轨时间轴、1x–256x 变速播放、热点事件标记、沉浸模式
- **书签与片段** — 标记关键时刻、创建/编辑片段、导出数据、热点自动建议
...
│       ├── components/       # 通用 UI 组件（设置、搜索、快捷键）
│       └── lib/              # API 客户端、WebSocket、i18n、工具
```
