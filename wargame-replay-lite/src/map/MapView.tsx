...
import { MobileQuickControls } from './MobileQuickControls';
...
export function MapView({ units, targetCamera: targetCameraProp, immersive = false, isMobile = false }: MapViewProps) {
  ...
  // Filter allHotspots by per-type filters for debug overlay
    ...
    <div className="absolute inset-0">
      ...
      {mapReady && mapRef.current && (
        <>
          ...
          {!immersive && <EventToastOverlay events={events} units={units} isMobile={isMobile} />}
          ...
          {activeHotspot && !selectedUnit && (
            <div className={`absolute ${isMobile ? 'left-2 top-2 px-2 py-1' : 'left-4 top-4 px-3 py-1.5'} z-10 bg-zinc-900/90 border rounded text-xs font-mono backdrop-blur-sm pointer-events-none ${focusMode.active ? 'border-amber-500/80' : 'border-amber-700/60'}`}>
          ...
          {/* Kill leaderboard — desktop only (hidden in immersive and mobile) */}
          {!immersive && !isMobile && <KillLeaderboard units={units} currentTs={currentTs} />}

          {/* Mobile quick controls (search / labels / hotspot) */}
          {isMobile && !immersive && <MobileQuickControls />}

          {/* Selected unit info panel */}
          {selectedUnit && (
            <div className={`absolute z-10 bg-zinc-900/90 border border-zinc-700 rounded text-xs font-mono text-zinc-200 backdrop-blur-sm ${
              isMobile ? 'bottom-2 left-2 right-2 px-2 py-1.5' : 'top-4 left-4 px-3 py-2 min-w-[200px]'
            }`}>
              <div className="flex items-center gap-2 mb-1">
                ...
                />
                <span className="font-bold truncate">{selectedUnit.name || `#${selectedUnit.id}`}</span>
                {!isMobile && <span className="text-zinc-500">({selectedUnit.team})</span>}
                {!selectedUnit.alive && (
                  <span className="text-red-500 text-[10px] font-bold">{t('kia')}</span>
                )}
                <span className="text-zinc-500 text-[10px] ml-auto shrink-0">HP {selectedUnit.hp} | {Math.round(ammoPercent(selectedUnit.ammo, selectedUnit.class))}%</span>
                <button
                  className="text-zinc-500 hover:text-zinc-200 text-xs shrink-0"
                  onClick={() => {
